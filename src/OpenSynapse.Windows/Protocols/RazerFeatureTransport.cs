...
public sealed class RazerFeatureTransport : IRazerFeatureTransport
{
        ...
        bool allowRemainingPacketsMismatch = false)
    {
        ...
        return ExecuteAsync(devicePath, request, deviceWait, cancellationToken, allowRemainingPacketsMismatch);
    }
        ...
        bool allowRemainingPacketsMismatch = false)
    {
        ...
        return ExecuteAsync(devicePath, request.ToArray(), deviceWait, cancellationToken, allowRemainingPacketsMismatch);
    }
        ...
        bool allowRemainingPacketsMismatch)
    {
        ...
        try
        {
            ...
            {
                var deviceHandle = NativeMethods.CreateFile(
                    devicePath,
                    NativeMethods.GENERIC_WRITE,
                    NativeMethods.FILE_SHARE_READ | NativeMethods.FILE_SHARE_WRITE,
                    IntPtr.Zero,
                    ...
                    0,
                    IntPtr.Zero);
                return (Handle: deviceHandle, Error: Marshal.GetLastWin32Error());
            }, cancellationToken);
            using var handle = openResult.Handle;
            ...
            for (var attempt = 1; attempt <= 5; attempt++)
            {
                ...
                await Task.Delay(deviceWait, cancellationToken);
                ...
                if (!RazerFeatureReport.Matches(request, response, allowRemainingPacketsMismatch))
                {
                    lastError = "设备返回了错序或校验失败的报告；请关闭 Synapse 后重试。";
                }
