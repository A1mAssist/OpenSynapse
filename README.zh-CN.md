...
> 当前版本 `v1.3.2` · Windows 11 x64 · 未签名

## 1.3.2 更新内容

本补丁改进了屏幕电源状态变化时的键盘灯光行为：

- Windows 进入睡眠或关闭屏幕时，键盘灯光会直接熄灭，不再停留在软件灯效的最后一帧。
- 提前处理睡眠广播，在 HID 会话仍可用时发送完整黑色矩阵帧。
- 合并重复的电源通知，并确保停止失败时仍释放 Software Mode 和灯光运行资源。
...
- `OpenSynapse-1.3.2-win-Setup.exe` 安装到当前用户目录并支持自动更新。
- `OpenSynapse-1.3.2-win-Portable.zip` 解压后即可运行。
